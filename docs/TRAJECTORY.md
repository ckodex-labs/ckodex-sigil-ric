...
| **C. Measurement gates** | | | |
| C1 | Coverage: `cargo-llvm-cov` wired in CI with `--fail-under-lines 70` no-regression floor + lcov artifact. **Baseline 74.01% → 79.29% lines / 80.76% regions** (74.32% fns) — regions now meet the 80% target, lines are ~0.7pt short. Closed gaps: `verify.rs` 0%→82%, `sigil-server` 0%→96%, `csr.rs` 25%→48% (`decode_chain_certs` extracted into a tested helper), `video.rs` 43%→98% (ffmpeg-generated `tiny_av.mp4` fixture exercises the success path), `tokenizer_ffi.rs` 66%→97% (8 tests over encode/decode/special-token/error paths against the linked Zig lib). Remaining gaps: sigstore network paths (`signer.rs`/`rekor_upload.rs`/`fetch.rs`, not offline-testable), `spectral/downmix.rs` 72%, `audio/transcript.rs` 85%. Ratchet floor as these get tests or explicit exclusions | `.github/workflows/ci.yml` Coverage gate; `cargo llvm-cov --workspace --summary-only` | **done** (tool wired); ratchet to 80% open |
