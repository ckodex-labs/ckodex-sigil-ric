...
| **C. Measurement gates** | | | |
| C1 | Coverage: `cargo-llvm-cov` wired in CI with `--fail-under-lines 70` no-regression floor + lcov artifact. **Baseline 74.01% → 77.48% lines** (73.00% fns, 79.01% regions) after A4 + facade/CSR tests — still below the 80% governance target. Closed gaps: `verify.rs` 0%→82%, `sigil-server` 0%→96%, `csr.rs` 25%→48% (`decode_chain_certs` extracted from the network path into a tested helper). Remaining gaps: sigstore network paths (`signer.rs`/`rekor_upload.rs`/`fetch.rs`, not offline-testable), `video.rs` 43%, `tokenizer_ffi.rs` 66%. Ratchet floor as these get tests or explicit exclusions | `.github/workflows/ci.yml` Coverage gate; `cargo llvm-cov --workspace --summary-only` | **done** (tool wired); ratchet to 80% open |
