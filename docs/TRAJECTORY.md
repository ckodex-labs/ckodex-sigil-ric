...
| G8 | Community-readiness paper cuts — the five frictions a first-time community user hits, fixed together: (1) verdicts now gate the process — `--fail-on deny` (default) exits 2 on Deny, `--fail-on flag` exits 1 on Flag, `--fail-on never` restores the old always-0, applied to `tokenize`/`scan`/`multimodal` and `perceive --analyze`; (2) `read_command_text` accepts `--input -` and auto-reads piped stdin (`cat payload \| sigil-cli scan`), with an interactive terminal erroring instead of blocking and empty piped stdin erroring rather than emitting a vacuous Allow; (3) `--input`/`--text` carry real help text; (4) `DecodeFailed` errors name the routed modality and suggest a fix (`cannot decode input as image: the input is UTF-8 text; try --modality document or --modality code`) without echoing input bytes; (5) adapter flags that cannot apply to the routed modality are rejected (`--ffmpeg-binary applies to --modality video; effective modality is image`) instead of silently ignored — validated post-routing so `--modality image` on an MP4 still accepts `--ffmpeg-binary`. Second pass: `mcp` honours `--fail-on`, `--format json` emits a `{"error":…}` envelope on stderr instead of a plain-text dump, render-compare covers multi-page PDFs (one `-f N -l N -singlefile` run per page, capped at 12 with `pages_capped` evidence), and the code adapter lexes `--` comments (`.sql`/`.lua` carriers, whitespace-gated so `i--` survives). 13 subprocess-level tests | `cli/types.rs` (`FailOn`, help text), `cli/helpers.rs` (`read_stdin`, `exit_on_verdict`), `cli/run.rs` (4 verdict arms), `cli/commands.rs` (flag table, `perceive_err`), `cli/run.rs` (JSON error envelope), `tests/cli_paper_cuts.rs` | **done** |
