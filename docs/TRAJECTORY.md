...
| F2 | Terminal-escape detection (`scan.terminal_escapes`, opt-in) — VT/ANSI control sequences in admitted text flagged at the boundary: OSC 52 clipboard hijack, OSC 8 hyperlink display/target smuggling, title/notification spoofing, ConEmu automation, CSI/DCS output forgery. Port boundary mirrors `perplexity.rs`: `TerminalSequenceScanner` trait in `sigil-core` (extraction/classification only); `sigil-vt` supplies the `libghostty-vt`-backed `GhosttyScanner` — Ghostty's fuzzed `osc::Parser` classifies the full OSC command taxonomy, CSI/DCS/ESC framing is a minimal span lexer (not emulation; full `Terminal`/`Screen` side-effect diff is a stated follow-up). Kernel owns severity (clipboard-write/ConEmu/DCS→`High`, hyperlink/title/notification/cwd-report→`Medium`, unclassified→`Low`), evidence text, and the verdict path (`FlagReason::TerminalEscape`). Enabled-without-scanner → `TerminalStatus::Skipped`; scanner error → `Failed` — visible evidence, never silent. `InputAssessment.terminal` carries the report; human renderer shows the status line. **Build boundary**: `libghostty-vt` needs rust ≥1.90 + zig 0.15.2 (its pinned ghostty commit) + network fetch of ghostty source — so `sigil-cli` gates injection behind non-default cargo feature `vt`; a no-vt build honestly records `Skipped` (verified end-to-end both ways). Workspace MSRV stays 1.78; CI runs a dedicated `vt` lane (rust 1.90 + zig 0.15.2), 1.78 lanes `--exclude sigil-vt`. 12 core + 6 scanner + 4 wiring tests; e2e via `sigil-cli --features vt scan` (OSC 52 → `Flag{TerminalEscape}`, span 11..31, `libghostty-vt 0.2.1 (osc)` named in evidence) | `crates/sigil-core/src/terminal.rs`, `crates/sigil-vt`, `policy.rs::TerminalEscapesPolicy`, `scan/run.rs::run_scan_with_engines`, `emit.rs`, `sigil-cli` `vt` feature, CI `vt` job | **done** |
