...
| **C. Measurement gates** | | | |
| C1 | Coverage: `cargo-llvm-cov` wired in CI with `--fail-under-lines 70` no-regression floor + lcov artifact. **Measured baseline 74.01% lines** (69.86% fns, 75.39% regions) — below the 80% governance target. Gap decomposition: `sigil-server` 0% (85 lines, facade untested), sigstore network paths 0% (`signer.rs`/`rekor_upload.rs`/`fetch.rs`, not offline-testable), `trust/verify.rs` 0%, `csr.rs` 25%, `video.rs` 43%, `tokenizer_ffi.rs` 66%. Ratchet floor as these get tests or explicit exclusions | `.github/workflows/ci.yml` Coverage gate; `cargo llvm-cov --workspace --summary-only` | **done** (tool wired); ratchet to 80% open |
