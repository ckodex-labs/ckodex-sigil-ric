...
| Terminal-escape injection (clipboard hijack OSC 52, hyperlink smuggling OSC 8, title/notification spoofing, CSI/DCS output forgery) | `sigil-core` terminal-escape detector (`scan.terminal_escapes`, scanner-trait boundary, opt-in) + `sigil-vt` Ghostty-backed scanner | `crates/sigil-core/src/terminal/tests.rs`, `crates/sigil-core/src/scan/tests.rs`, `crates/sigil-vt/src/lib.rs` unit tests |
