...
| Video-borne instruction injection (painted frames, spoken audio track) | `sigil-perception` video adapter stream extraction (pinned ffmpeg → `AudioAdapter` + per-frame OCR channels) | `crates/sigil-perception/tests/stream_extract.rs`, `crates/sigil-perception/src/video.rs` unit tests |
