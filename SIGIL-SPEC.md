...
Sequence extraction comes from a `TerminalSequenceScanner` trait — emulator code never lives in the kernel. The `sigil-vt` crate supplies the built-in implementation (`VtScanner`): a purpose-built span lexer frames CSI/DCS/OSC/ESC sequences and a numeric-selector table classifies OSC commands — the table mirrors ghostty's `osc.zig` taxonomy (pinned commit, reviewed against source) including ConEmu `9;N` subcommands, OSC 8 start/end, and the rxvt/iTerm2/kitty extension channels. Sequences become *findings* — the kernel owns severity mapping (clipboard-write/ConEmu automation/extension-channel/DCS → `High`; hyperlink/title/notification/cwd-report/mode/file-drop → `Medium`; unclassified or unknown OSC → `Low` — a control sequence in admitted text is reportable regardless of whether it can be named) and the verdict path (`FlagReason::TerminalEscape`). Enabled-without-scanner records `TerminalStatus::Skipped`; scanner errors record `Failed` — visible evidence, never silent. Disabled by default; enable via `[scan.terminal_escapes] enabled = true`. The `sigil-vt` `conformance` feature cross-checks the owned table against `libghostty-vt`'s fuzzed parser in CI (test-only dep — never in the runtime path).
