...
Sequence extraction comes from a `TerminalSequenceScanner` trait — emulator code never lives in the kernel. The `sigil-vt` crate supplies the built-in implementation (`VtScanner`): a purpose-built span lexer frames CSI/DCS/OSC/ESC sequences, a numeric-selector table classifies OSC commands (mirroring ghostty's `osc.zig` taxonomy at pinned commit — ConEmu `9;N` subcommands, OSC 8 start/end, rxvt/iTerm2/kitty extension channels), and an ECMA-48/xterm CSI table decodes operation names (`erase_line`, `sgr_conceal`, `cursor_up`, `alt_screen`, …). The scanner also emits synthesized `Pattern` findings — `repaint_overwrite` — driven by a *bounded virtual window* (`window.rs`, a sliding 200×48 cell grid, not a terminal emulator): writes, cursor moves, erases, line/scroll shifts, and alternate-screen toggles are replayed against the grid, and a finding fires when the displayed byte stream *diverges* from what the window shows — a byte written over a cell already showing a different byte (cursor-back/BS overstrike), or a row whose pre-erase snapshot differs from its final non-blank content (erase+rewrite, displaced rewrite). Byte-identical rewrites and rows left blank after erase stay silent; rows are evaluated at scroll eviction so divergence survives the slide. Sequences become *findings* — the kernel owns severity mapping (clipboard-write/ConEmu automation/extension-channel/DCS → `High`; hyperlink/title/notification/cwd-report/mode/file-drop OSC and concealed-text SGR/scrollback-erase/alt-screen CSI/repaint patterns → `Medium`; unclassified or unknown sequences → `Low` — a control sequence in admitted text is reportable regardless of whether it can be named) and the verdict path (`FlagReason::TerminalEscape`). Enabled-without-scanner records `TerminalStatus::Skipped`; scanner errors record `Failed` — visible evidence, never silent. Disabled by default; enable via `[scan.terminal_escapes] enabled = true`. The `sigil-vt` `conformance` feature cross-checks the owned OSC table against `libghostty-vt`'s fuzzed parser in CI and replays every modeled CSI op through Ghostty's real `Terminal` for cell-for-cell grid parity (test-only dep — never in the runtime path).
